...
(function ($) {
	...
	var defaults = {
		// 기본적으로 floplan.svg라는 로컬 .svg 파일을 찾습니다.
		'maps': [{'path': 'floorplan.svg', 'id': 'map.1'}],
		// 경로 형식 지정
		'path': {
			color: 'red', 
			radius: 10, // 경로에 적용할 반경(픽셀)
			speed: 8, // 경로 도출 속도
			width: 3 // 경로 너비(픽셀)
		},
		// 기본 시작점의 도어 식별자
		...
		// wayfind 초기화에 지정된 경우
		// 맵이 로드되는 즉시 이 지점으로 이동합니다. 초기화 가능
		// 함수 또는 문자열로(시작점과의 일관성을 위해)
		'endpoint': false,
		//계단을 통한 라우팅 제어
		'accessibleRoute': false,
		//시작 시 표시해야 하는 지도에 대한 식별자를 제공합니다. 그렇지 않으면 어레이에 첫 번째 맵이 표시되도록 기본값이 지정됩니다.
		'defaultMap': function () {
			return 'map.1';
		},
	...
	$.fn.wayfinding = function (action, options) {
		...
		var passed = options,
			...
			obj, // 사용할 jQuery 객체
			maps, // 매번 옵션에서 작성된 지도 배열
			defaultMap, // 시작 층 
			startpoint, // options.startpoint 값 또는 함수 값의 결과
			portalSegments = [], // 포털이 조립될 때까지 포털 조각을 저장하는 데 사용되었고, 그리고 나서 이것은 버려집니다.
			drawing,
			solution,
			result; // jQuery가 아닌 결과를 반환하는 데 사용됨

		// 제공된 옵션, 이전 설정 또는 기본값을 기준으로 옵션 설정
		function getOptions(el) {

			var optionsPrior = el.data('wayfinding:options'), // 이전 설정 로드 시도
				dataStorePrior = el.data('wayfinding:data'); // load data

			drawing = el.data('wayfinding:drawing'); // 그려진 경로를 로드합니다(있는 경우).
